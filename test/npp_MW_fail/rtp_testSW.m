...
hapod = 0;  % specify apodization

if true
  dstr = '3 Mar 2019'; fstr = 'pre_fail';
  p1 = '/home/motteler/cris/move_data/npp_scrif/2019/062';
% gn = 'GCRSO-SCRIF_npp_d20190303_t0407119_e0415097_b38062_c20190507191704252101_n%bc_ops.h5';
...
% p2 = '/asl/cris/ccast/sdr45_npp_HR/2019/062';
  p2 = './ccast_ref_eng_a2_new_ict/sdr45_npp_HR/2019/062';
  p3 = './ccast_a4_eng_a2_new_ict/sdr45_npp_HR/2019/062';
% gc = 'CrIS_SDR_npp_s45_d20190303_t0406080_g042_v20a.mat';
...
% gn = 'GCRSO-SCRIF_npp_d20190421_t0400319_e0408297_b38757_c20190506160924977285_noac_ops.h5';
...
% gc = 'CrIS_SDR_npp_s45_d20190421_t0400080_g041_v20a.mat';
...
vSW = d2.vSW;  vSW = d2.vSW;  % ccast and noaa freq
...
for i = 1 : length(cn)
  ...
  rad1 = d1.ES_RealSW(:, jFOV, rn(i), cn(i));
  rad2 = d2.rSW(:, jFOV, rc(i), cc(i));
  rad3 = d3.rSW(:, jFOV, rc(i), cc(i));
  ...
  bt1 = [bt1, rad2bt(vSW, rad1)];  % noaa a4
  bt2 = [bt2, rad2bt(vSW, rad2)];  % ccast ref
  bt3 = [bt3, rad2bt(vSW, rad3)];  % ccast a4

  cr1 = [cr1, d1.ES_ImaginarySW(:, jFOV, rn(i), cn(i))];
  cr2 = [cr2, d2.cSW(:, jFOV, rc(i), cc(i))];
  cr3 = [cr3, d3.cSW(:, jFOV, rc(i), cc(i))];
...
% [jx1, jx2] = seq_match(vSW, vchan, 0.05);
...
[im, ir] = seq_match(tm, tr, 0.01);
% [jx1, jx2] = seq_match(vSW, cfreq, 0.05);
[jx1, jx2] = seq_match(vSW, vchan, 0.05);
vx1 = vSW(jx1);
% vx2 = cfreq(jx2);
nm = length(im);
...
axis([2230, 2290, -3, 3]);
title('sarta comparisons')
legend('noaa a4 minus sarta', 'ccast ref minus sarta', ...
       'ccast a4 minus sarta', 'location', 'northwest');
grid on; zoom on
s3 = sprintf('SW_%s_sarta_fov%d_sd%d', fstr, jFOV, jdir);
% saveas(gcf, s3, 'png')
