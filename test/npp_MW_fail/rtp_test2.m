...
vLW = d2.vLW;  vSW = d2.vSW;  % ccast and noaa freq
...
for i = 1 : length(cn)
  ...
  rad1 = d1.ES_RealLW(:, jFOV, rn(i), cn(i));
  rad2 = d2.rLW(:, jFOV, rc(i), cc(i));
  rad3 = d3.rLW(:, jFOV, rc(i), cc(i));
  ...
  cr1 = [cr1, d1.ES_ImaginaryLW(:, jFOV, rn(i), cn(i))];
  cr2 = [cr2, d2.cLW(:, jFOV, rc(i), cc(i))];
  cr3 = [cr3, d3.cLW(:, jFOV, rc(i), cc(i))];
...
% [jx1, jx2] = seq_match(vLW, vchan, 0.05);
...
[im, ir] = seq_match(tm, tr, 0.01);
% [jx1, jx2] = seq_match(vLW, cfreq, 0.05);
[jx1, jx2] = seq_match(vLW, vchan, 0.05);
vx1 = vLW(jx1);
% vx2 = cfreq(jx2);
nm = length(im);
...
grid on; zoom on
s3 = sprintf('LW_%s_sarta_fov%d_sd%d', fstr, jFOV, jdir);
% saveas(gcf, s3, 'fig')
