...
% p2 = './ccast_ref_eng_a2_new_ict/sdr45_npp_HR/2019/063';
  p2 = './ccast_a4_eng_a2_new_ict/sdr45_npp_HR/2019/063';
  p3 = './ccast_a4_uw_a2_new_ict/sdr45_npp_HR/2019/063';
% gc = 'CrIS_SDR_npp_s45_d20190304_t1006080_g102_v20a.mat';
  gc = 'CrIS_SDR_npp_s45_d20190304_t0906080_g092_v20a.mat';
else
...
vMW = d2.vMW;   % ccast MW freq, also used for noaa
...
% if mod(r1(i),2) ~= jdir, continue, end
  ...
  rad1 = d1.ES_RealMW(:, jFOV, r1(i), c1(i));
  rad2 = d2.rMW(:, jFOV, r2(i), c2(i));
  rad3 = d3.rMW(:, jFOV, r2(i), c2(i));
  bt1 = [bt1, rad2bt(vMW, rad1)];  % noaa a4
  bt2 = [bt2, rad2bt(vMW, rad2)];  % ccast ref
  bt3 = [bt3, rad2bt(vMW, rad3)];  % ccast a4

  cr1 = [cr1, d1.ES_ImaginaryMW(:, jFOV, r1(i), c1(i))];
  cr2 = [cr2, d2.cMW(:, jFOV, r2(i), c2(i))];
  cr3 = [cr3, d3.cMW(:, jFOV, r2(i), c2(i))];
...
figure(1)
subplot(3,1,1)
plot(vMW, btm1, vMW, btm2, vMW, btm3)
axis([1200, 1750, 200, 300])
% title(sprintf('%s MW BT mean, FOV %d sweep %d', dstr, jFOV, jdir))
title(sprintf('%s MW BT mean, FOV %d', dstr, jFOV))
legend('noaa a4', 'ccast ref', 'ccast a4', 'location', 'northeast')
ylabel('BT (K)')
grid on; zoom on
...
figure(2)
subplot(2,1,1)
plot(vMW, crm1, vMW, crm2, vMW, crm3)
axis([1200, 1750, -0.25, 0.25])
% title(sprintf('%s MW imag resid mean, FOV %d sweep %d', dstr, jFOV, jdir))
title(sprintf('%s MW imag resid mean, FOV %d', dstr, jFOV))
legend('noaa a4', 'ccast ref', 'ccast a4')
ylabel('mw sr-1 m-2')
grid on; zoom on

subplot(2,1,2)
plot(vMW, crs1, vMW, crs2, vMW, crs3)
axis([1200, 1750, 0, 0.25])
