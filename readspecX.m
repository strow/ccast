%
% NAME
%   readspecX - get spectra from a CrIS RDR interferogram
%
% SYNOPSIS
%   [spec, freq, opts] = readspecX(igm, band, opts);
%
% INPUTS
...
% DISCUSSION
%
%   This version is modified to work with high-res igms taken
%   on 22-23 Feb 2012
%
...
%   the frequency scale and other values returned do.  readspecX
...
function [spec, freq, opts] = readspecX(igm, band, opts)
...
b2.npts = 1037;    % number of decimated points
b2.c1ind = 824;   % first channel index
b2.vhack = 1;

% band 3 params
b3.df = 26;	  % decimation factor
b3.npts = 797;    % number of decimated points
b3.c1ind = 192;    % first channel index
b3.vhack = 4;
...
if nargin == 3
  ...
  for i = 1 : length(optvar)
    vname = optvar{i};
    if exist(vname, 'var')
      % fprintf(1, 'readspecX: setting %s\n', vname)
      eval(sprintf('%s = opts.%s;', vname, vname));
    else
      fprintf(1, 'readspecX: unknown option variable %s\n', vname);
    end
  end
end
