%
% checkRDR4 -- validate MIT RDR data 
%
...
% % check that band 1 FOR agrees for all FOVs
% t1 = max(tmp1FOR);
% t2 = min(tmp1FOR);
% t1(isnan(t1)) = -1;
% t2(isnan(t2)) = -1;
% if ~isequal(t1,t2)
%    fprintf(1, 'Warning: LW FOV FORs differ, index %d file %s\n', ri, rid);
% end
  ...
  if min(diff(ngTime)) < 10
    fprintf(1, 'WARNING: NGAS time step too small, index %d file %s\n', ri, rid);
    find(diff(ngTime) < 10)'
  else
    ...
    if ~isequal(squeeze(igmLW(1:866,nfov,ix)), ngIGM)
      fprintf(1, 'WARNING: NGAS equality test failed, index %d file %s\n', ri, rid);
    end
  end
