...
% idata.LWIT(1:866,FOV,packet_counter.LWIT(FOV)) = bit_unpack_LW(idata_packed);        
% qdata_packed = fread(fid,(header.packet_length-28)/4,'uint16');
% qdata.LWIT(1:866,FOV,packet_counter.LWIT(FOV)) = bit_unpack_LW(qdata_packed);

if header.packet_length==2488
idata_packed = fread(fid,(header.packet_length-28)/4,'uint16');
idata.LWIT(1:866,FOV, packet_counter.LWIT(FOV)) = bit_unpack_LW2488(idata_packed);
qdata_packed = fread(fid,(header.packet_length-28)/4,'uint16');
qdata.LWIT(1:866,FOV, packet_counter.LWIT(FOV)) = bit_unpack_LW2488(qdata_packed);
elseif header.packet_length==2816
 idata_packed = fread(fid,(header.packet_length-28)/4,'uint16');
idata.LWIT(1:866,FOV, packet_counter.LWIT(FOV)) = bit_unpack_LW2816(idata_packed);
qdata_packed = fread(fid,(header.packet_length-28)/4,'uint16');
qdata.LWIT(1:866,FOV, packet_counter.LWIT(FOV)) = bit_unpack_LW2816(qdata_packed);
else
