...
function read_packet_IM_Tel_dwell_data

global fid  VERBOSE timeval idata qdata data ...
packet_counter packet header sweep_direction FOR diagint
  
packet_counter.IM_Tel_dwell = packet_counter.IM_Tel_dwell + 1;
L=(header.packet_length-8)/2;
               
if data.read.IM_Tel_dwell==1
...
buf=fread(fid,4,'uint16=>uint16');
    ...
    L=L-8;% 5 read alredy,3 at end
    if(~(L==560));disp('IM TelDwell packet is wrong lenght');return;end
 ...
 % store in MySQL database im_dwell_data
%     mym(['replace INTO im_dwell_data(time,status_flag,error_flag,address,sample) VALUES ("{Sn}","{Si}","{Si}","{M}","{M}")'],...
%     fix(timeval*1.e10),status_flag,error_flag,address,sample);
   
    data.IM_Tel_dwell.time=timeval;
    data.IM_Tel_Dwell.address=address;
    data.IM_Tel_dwell.sample=sample;  
    data.IM_Tel_dwell.status_spare=status_spare;      
    data.IM_Tel_dwell.status_flag=status_flag;
    data.IM_Tel_dwell.error_flag=error_flag;
