...
function B = cris_ICTradModel_CCAST(band,wn,T_ICT,T_CRIS,ICT_Param,eFlag,eICT,ssmFlag,ssmOffset)
...
%             cris_calc_instrument_temps_CCAST.m and converted into an array of structures.  
...
%
%    3/13/2009: cris_ICTradModelTVAC3_dct.m changed view factors to be equal to values given in 
% 		IR15565_Step112_ ICT_Emissivity_Retrieval_10262008.xls and made T_notch an input
% 		
%    4/06/2009: JKT, modded cris_ICTradModelTVAC3.m code to match cris_ICTradModelTVAC3_dct.m version emailed on 3/17/2009
%
%    7 Nov 2011: DCT: renamed to cris_ICTradModel_CCAST.m and made numerous edits regarding input format and options for ICT emissivity and SSM Baffle offset temps.
%
...
if ssmFlag == 1   % Pull offsets from 4-min-eng-packet and interpolate to this time in the orbit.
  ...
  % FIXME - could be implemented more accurately.  Currently uses a hard-coded North pole crossing time and the orbital
  % period to compute the "orbit time".  Could/should be changed to compute the latest North pole crossing time using the
  % TLE and nagivation sw.

  north_pole_dnum = datenum(2011,11,11,6,23,13.4);   % this was obtained from running Fred' code "when" for the north pole
  sec_per_day = 24*60*60;
  % orbit_time = mod((north_pole_dnum - T_CRIS.dnum)*sec_per_day,ICT_Param.Orbital_Period);
  orbit_time = mod((north_pole_dnum - T_CRIS.dnum)*sec_per_day,6085.3);
% CHECK THIS vs. ICT_Param.Orbital_Period when have real data, and use ICT_Param value if ok.

  % Orbit time values taken from the ATBD.  The default orbit period is listed as 6060 seconds, but Fred's code is giving 
  % 6085.3 seconds, and I've added another (last) point at 6085.3.
  orbit_time_vector = [0 288.6 577.1 865.7 1154.3 1442.9 1731.4 2020 2308.6 2597.1 2885.7 3174.3 ...
                        3462.9 3751.4 4040 4328.6 4617.1 4905.7 5194.3 5482.9 5771.4 6085.3];
