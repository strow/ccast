function [oafreq,oasrf] = oaffov(fgrid,fchan,opd,thetac,hfov,nslice,N);

%
%  function [oafreq,oasrf] = oaffov(fgrid,fchan,opd,thetac,hfov,nslice);
%
...
for i=1:nslice
   ...
   ind=find(A_i ~= 0);
   slice(ind)=W_i*sin(A_i(ind))./(N*sin(A_i(ind)/N));
   oasrf=oasrf + slice;
   %
