%
% testESpoly  - plots and fit for ES vs rad matchups
%
...
P1re = zeros(3, 9);            P1im = zeros(3, 9);
P2re = zeros(3, 9);            P2im = zeros(3, 9);
rms1re = zeros(9, 1);          rms1im = zeros(9, 1);
rms2re = zeros(9, 1);          rms2im = zeros(9, 1);

% loop on FOVs, do linear fits
for fi = 1 : 9
  P1re(:, fi) = polyfit(rad1(fi,:)', ES1re(fi, :)', 2);
  P1im(:, fi) = polyfit(rad1(fi,:)', ES1im(fi, :)', 2);
  P2re(:, fi) = polyfit(rad2(fi,:)', ES2re(fi, :)', 2);
  P2im(:, fi) = polyfit(rad2(fi,:)', ES2im(fi, :)', 2);
